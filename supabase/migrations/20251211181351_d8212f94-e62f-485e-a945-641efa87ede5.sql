-- Função para fechar negociação quando fornecedor recusa via código de acesso
...
BEGIN
  ...
  -- Registrar mensagem de recusa no chat
  INSERT INTO mensagens_negociacao (
    ...
    p_fornecedor_id,
    p_numero_item,
    '❌ Fornecedor recusou a negociação e encerrou o item.',
    'fornecedor'
  );
...
-- Permitir que qualquer usuário possa chamar a função (validação é feita dentro da função)
GRANT EXECUTE ON FUNCTION public.fechar_negociacao_fornecedor(uuid, integer, uuid, text) TO anon, authenticated;