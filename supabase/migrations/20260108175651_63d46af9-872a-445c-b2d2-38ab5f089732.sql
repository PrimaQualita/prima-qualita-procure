-- Corrigir função de auditoria para anexos_processo_compra (usa processo_compra_id, não cotacao_id)
...
BEGIN
  ...
  CASE TG_TABLE_NAME
    ...
    -- Anexos de processo de compra (usa processo_compra_id, não cotacao_id)
    WHEN 'anexos_processo_compra' THEN
      IF TG_OP = 'DELETE' THEN
        -- Buscar número do processo pelo processo_compra_id
        SELECT numero_processo_interno INTO v_numero_processo
        FROM processos_compras
        WHERE id = OLD.processo_compra_id;
        
        -- Determinar nome bonito do documento
    ...
    WHEN 'cotacao_respostas_fornecedor' THEN
      IF TG_OP = 'DELETE' THEN
        SELECT c.titulo_cotacao, p.numero_processo_interno, f.razao_social
        INTO v_titulo, v_numero_processo, v_fornecedor
    ...
    WHEN 'analises_compliance' THEN
      IF TG_OP = 'DELETE' THEN
        v_detalhes := jsonb_build_object(
          ...
          'protocolo', COALESCE(OLD.protocolo, 'N/A'),
      ...
      ELSE
        v_detalhes := jsonb_build_object(
          ...
          'protocolo', COALESCE(NEW.protocolo, 'N/A'),
          'tipo', 'Análise de Compliance'
        );
      END IF;
