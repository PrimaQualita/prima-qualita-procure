...
BEGIN
  ...
  CASE TG_TABLE_NAME
    WHEN 'processos_compras' THEN
      IF TG_OP = 'DELETE' THEN
        SELECT cg.nome_contrato INTO v_contrato_gestao
        FROM contratos_gestao cg WHERE cg.id = OLD.contrato_gestao_id;
      ...
      ELSE
        SELECT cg.nome_contrato INTO v_contrato_gestao
        FROM contratos_gestao cg WHERE cg.id = NEW.contrato_gestao_id;
        v_detalhes := jsonb_build_object(
    ...
    WHEN 'anexos_processo_compra' THEN
      IF TG_OP = 'DELETE' THEN
        SELECT p.numero_processo_interno, cg.nome_contrato
        INTO v_numero_processo, v_contrato_gestao
    ...
    WHEN 'atas_selecao' THEN
      IF TG_OP = 'DELETE' THEN
        SELECT s.titulo_selecao, p.numero_processo_interno, cg.nome_contrato
        INTO v_selecao, v_numero_processo, v_contrato_gestao
    ...
    WHEN 'planilhas_consolidadas' THEN
      IF TG_OP = 'DELETE' THEN
        SELECT c.titulo_cotacao, p.numero_processo_interno, cg.nome_contrato
        INTO v_titulo, v_numero_processo, v_contrato_gestao
        FROM cotacoes_precos c
        JOIN processos_compras p ON c.processo_compra_id = p.id
        LEFT JOIN contratos_gestao cg ON p.contrato_gestao_id = cg.id
        WHERE c.id = OLD.cotacao_id;
  ...
  INSERT INTO audit_logs (
    entidade, entidade_id, acao, detalhes, usuario_id, usuario_nome, usuario_tipo
  ) VALUES (
    TG_TABLE_NAME, v_entidade_id, v_acao, v_detalhes, v_usuario_id, v_usuario_nome, 'interno'
  );
