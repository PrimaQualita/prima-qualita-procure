...
export async function gerarAtaSelecaoPDF(selecaoId: string): Promise<{ url: string; nome: string; protocolo: string }> {
  ...
  // Identificar vencedores: primeiro lance de cada item (pelos lances)
  ...
  // FALLBACK: Para itens/lotes SEM LANCES, buscar vencedor pela PROPOSTA ORIGINAL classificada
  // Buscar propostas com valores para identificar vencedores por proposta
  const { data: propostasComValores } = await supabase
  ...
  // Buscar lotes para mapeamento lote_id -> numero_lote
  let loteIdToNumeroGlobal = new Map<string, number>();
  if (criterioJulgamento === 'por_lote' && selecao.cotacao_relacionada_id) {
    const { data: lotesData } = await supabase
      .from('lotes_cotacao')
      .select('id, numero_lote')
      .eq('cotacao_id', selecao.cotacao_relacionada_id);
    if (lotesData) {
      loteIdToNumeroGlobal = new Map(lotesData.map(l => [l.id, l.numero_lote]));
    }
  }
  ...
  // ============= 12 - ENCERRAMENTO =============
  // Verificar espaço para título + texto de encerramento + certificação digital (~100 pixels)
  // para evitar que a seção comece no final de uma página e quebre imediatamente
  checkNewPage(100);
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
...
export async function atualizarAtaComAssinaturas(ataId: string): Promise<void> {
  ...
  console.log('>>> Margem lateral (1.5mm):', sideMargin, 'pontos');
  
  // Adicionar logo expandido no topo da nova página (com margem de 1.5mm igual às páginas anteriores)
  ...
  const logoExpandidoWidth = pageWidth - (sideMargin * 2);
  ...
  const checkNewPage = async (espacoNecessario: number) => {
    if (currentY - espacoNecessario < footerLimit) {
      ...
      // Adicionar logo expandido no topo da nova página (com margem de 1.5mm)
      page.drawImage(logoExpandidoImage, {
        x: sideMargin,
        y: pageHeight - logoExpandidoHeight,
        width: logoExpandidoWidth,
  ...
  // Adicionar rodapé expandido no final de todas as páginas com termo de aceite (com margem de 1.5mm)
  ...
  const rodapeExpandidoWidth = pageWidth - (sideMargin * 2);
