...
export async function gerarPlanilhaHabilitacaoPDF(
    ...
    didDrawCell: (data) => {
      ...
      if (data.section === 'head' && data.column.dataKey && typeof data.column.dataKey === 'string' && data.column.dataKey.startsWith('fornecedor_')) {
        ...
        if (resposta && resposta.rejeitado) {
          ...
          const linhasTexto = doc.splitTextToSize(textoCompleto, data.cell.width - 2);
          const alturaLinha = 3;
          const yInicio = data.cell.y + (data.cell.height - (linhasTexto.length * alturaLinha)) / 2 + alturaLinha;
      ...
      // Descrição usa renderização nativa do autoTable (overflow: 'linebreak') - sem lógica manual
    },
    didDrawPage: (data) => {
  ...
  console.log("📋 HABILITAÇÃO - Total respostas:", respostas.length);
  console.log("📋 HABILITAÇÃO - Empresas inabilitadas encontradas:", empresasInabilitadas.length);
  empresasInabilitadas.forEach((e, i) => {
    console.log(`📋 HABILITAÇÃO - Empresa ${i + 1}:`, e.fornecedor.razao_social, "rejeitado:", e.rejeitado, "itens_rejeitados:", e.itens_rejeitados);
  });
  
  // CRÍTICO: Obter posição Y final da tabela
