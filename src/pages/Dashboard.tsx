  ...
  const loadData = async () => {
    try {
      const [contratosRes, processosRes, selecoesRes, fornecedoresRes, complianceRes, contratosTerc, pcRes, cotRes] = await Promise.all([
        ...
        supabase.from("fornecedores").select("id, created_at, data_cadastro, data_validade_certificado, status_aprovacao, razao_social, nome_fantasia, email").limit(5000),
        ...
        supabase.from("cotacoes_precos").select("id, enviado_compliance, respondido_compliance, processo_compra_id, processos_compras(contrato_gestao_id)").limit(5000),
      ]);
